...
$(document).ready(function() {
    var checkboxes = $(".delete_member_cb"),
        submitButt = $("#member_delete_btn");
...
$password_policy = function(){
    ...
    $.ajax({
        type: 'GET',
        url: '/aws_actions/load_password_policy',
        data: {},
        dataType: 'html',
        success: function(data){

            $('div#password-policy').html(data);
            //alert(($('.password-policy').text() != "No validation"))
            if ($('.password-policy').text() != "No validation")   {
            var length = parseInt( $('#minimum-password-length').text())
            ...
            $("#new_user").validate({
                ...
                rules: {
                    'user[iam_password]': {
                        required: true,
                        minlength: parseInt( $('#minimum-password-length').text()),
                        atleastonelowercase: ( $('#require_lowercase_characters').text()),
                        atleastoneuppsercase: ( $('#require_uppercase_characters').text()),
                        atleastonenumber: ( $('#require_numbers').text()),
                        atleastonesymbol: ( $('#require_symbols').text())
                    }
                },
        ...
        error: function(e){
            ...
            $('div#password-policy').html("<div class='well'><center>Some error occured while loading password policy. Please verify your AWS creds.</center></div>");
        },
        beforeSend: function(){
            $('div#password-policy').html("<div><center><img src='/assets/loading.gif' style='align:center'/></center></div>");
        }
    });
};
  ...
  $(document).ready(function() {
      ...
      $.validator.addMethod("atleastonesymbol", function (value, element) {
              return /^.*[_\W].*$/.test(value);
          }, "At least one symbol."
      );

      $.validator.addMethod("atleastonelowercase", function (value, element) {
              return /^.*[a-z].*$/.test(value);
          }, "At least one lower case  Characters ."
      );

      $.validator.addMethod("atleastoneuppsercase", function (value, element) {
              return /^.*[A-Z].*$/.test(value);
...
$list_groups = function( ){
    $.ajax({
        type: 'GET',
        url: '/aws_actions/list_all_groups',
        data: {},
        dataType: 'html',
        success: function(data){
            $('div#list-groups').html(data);
        },
        error: function(e){
            $('div#list-groups').html("<div class='well'><center>Some error occured while loading groups. Please verify your AWS creds.</center></div>");
        },
        beforeSend: function(){
            $('div#list-groups').html("<div><center><img src='/assets/loading.gif' style='align:center'/></center></div>");
        }
    });
};
...
$(function() {
   $("body").on("ajax:success","a.instance-detail-link", function(e, data, status, xhr) {
        $('div#div-instance-detail').html(data);
  }).on("ajax:error", function(e, data, status, xhr) {
            $('div#div-instance-detail').html("<div class='well'><center>Some error occured while loading groups. Please verify your AWS creds.</center></div>");
  }).on("ajax:beforeSend","a.instance-detail-link", function(e, data, status, xhr) {
  	 $("div#instance-detail").show();
     $('div#div-instance-detail').html("<div><center><img src='/assets/loading.gif' style='align:center'/></center></div>");
  });
});
