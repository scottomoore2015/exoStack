class AwsActionsController < ApplicationController
  ...
  def load_instances
    ...
    # @platform = response.reservation_set.last.instances_set.first.platform
  ...
  def show
    response = @ec2.client.describe_instances(:instance_ids => ["#{params[:id]}"])
    @instance = response.reservation_set.map(&:instances_set).flatten!
  end
  ...
  def create_schedule
    begin
      ...
      flash[:error] = "Some error occured while processing your request. Please verify your AWS creds or region"
  ...
  def load_password_policy
    iam = AWS::IAM.new(
        :access_key_id => current_user.access_key,
        :secret_access_key => current_user.secret_token,
        :region => current_user.default_region)
    @policy =  iam.account_password_policy
    #@policy ={:minimum_password_length=>15, :require_symbols=>true, :require_numbers=>true, :require_uppercase_characters=>true, :require_lowercase_characters=>true}
    respond_with @policy
  end
