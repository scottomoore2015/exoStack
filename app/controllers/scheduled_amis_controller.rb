class ScheduledAmisController < ApplicationController
  ...
  def index
    @scheduled_amis = ScheduledAmi.where( :user_id => User.get_all_members(current_user))
    respond_with @scheduled_amis
  end

  def show
     @scheduled_ami = ScheduledAmi.find(params[:id])
     if ((current_user.id == @scheduled_ami.user_id) || (current_user.id == @scheduled_ami.user.admin_id))
      respond_with @scheduled_ami
      else
      flash[:alert] = "You are not allowed to view this scheduled amis"
  ...
  def update
    @scheduled_ami = ScheduledAmi.find(params[:id])
     @scheduled_ami.update(ami_params)   
       flash[:notice] = "Ami Updated Successfully"
       redirect_to @scheduled_ami
