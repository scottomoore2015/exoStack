...
class ScheduledSnapshot < ActiveRecord::Base
  ...
  after_update :set_crontab,:check_scheduling_date
  ...
  def set_crontab
    cron_array = case frequency
      when "None"
        [start_time.min, start_time.hour-self.user.time_zone.to_i, start_date.mday, start_date.month, "*"]
      when "Hourly"
        ["0", time_of_day.join(','), "* * *"]
      when "Daily"
        [start_time.min, start_time.hour-self.user.time_zone.to_i, "* *", day_of_week.join(',')]
      when "Weekly"
        [start_time.min, start_time.hour-self.user.time_zone.to_i, "* *", start_date.wday]
      when "Monthly"
        [start_time.min, start_time.hour-self.user.time_zone.to_i, start_date.mday, month_of_year.join(','), "*"]
    end
