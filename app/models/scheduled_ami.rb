class ScheduledAmi < ActiveRecord::Base
  ...
  after_update :set_crontab, :schedule_ami
  ...
  def set_crontab
    cron_array = case frequency
        when "Daily"
          [event_time.min, event_time.hour-self.user.time_zone.to_i, "* * *"]
        when "Weekly"
          [event_time.min, event_time.hour-self.user.time_zone.to_i, "* *", self[:day_of_week]]
        when "Monthly"
          [event_time.min, event_time.hour-self.user.time_zone.to_i, day_of_month, "* *"]
      end
