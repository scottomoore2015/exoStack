  ...
  def set_event_crontab
    self.events.each do |event|
      cron_array = case event.frequency
        when "Daily"
          [event.event_time.min, event.event_time.hour-event.schedule.user.time_zone.to_i, "* * *"]
        when "Weekly"
          [event.event_time.min, event.event_time.hour-event.schedule.user.time_zone.to_i, "* *", event[:day_of_week]]
        when "Monthly"
          [event.event_time.min, event.event_time.hour-event.schedule.user.time_zone.to_i, event.day_of_month, "* *"]
      end
